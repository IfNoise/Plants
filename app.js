...
app.use("/api/v1", require("./routes/metrics.routes.js"));
    ...
    try {
      ...
      for (const file of req.files) {
        try {
          ...
          console.log(
            `[UPLOAD] ✓ Thumbnail created successfully: ${file.filename}`,
          );
          results.push({ filename: file.filename, success: true });
        } catch (fileError) {
          console.error(
            `[UPLOAD] ✗ Error processing ${file.filename}:`,
            fileError.message,
          );
          results.push({
            filename: file.filename,
      ...
      console.log(
        `[UPLOAD] Complete: ${successCount}/${req.files.length} thumbnails created`,
      );
      ...
      res
        .status(500)
        .send({ message: "Error creating thumbnails", error: error.message });
    }
  },
);
  ...
  try {
    ...
    await Promise.all(
      photos.map(async (photo) => {
        const filename = photo.src?.includes("gallery/")
          ? photo.src.split("/")[1]
          : photo.src;
