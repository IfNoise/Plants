    ...
    <>
      <Card>
        <CardHeader
          ...
          action={
            <Checkbox icon={<LockIcon />} checkedIcon={<LockOpenIcon />} />
          }
        />
        <CardContent>
          <div>
            <strong>Тип:</strong>{" "}
            {getDeviceType() === "rtu" ? "Modbus RTU" : "Modbus TCP"}
          </div>
          {getDeviceType() === "rtu" && device.unitId && (
            <div>Unit ID: {device.unitId}</div>
          )}
          {options.timeout && <div>Timeout: {options.timeout}ms</div>}
          {device.portsCount && <div>Портов: {device.portsCount}</div>}
        </CardContent>
        <CardActions>
          <Button
            variant="outlined"
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Вы уверены, что хотите удалить устройство &quot;{name}&quot;? Это
